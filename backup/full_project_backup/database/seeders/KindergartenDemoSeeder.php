...
class KindergartenDemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (User::where('email', 'admin@kindergarten.ye')->exists()) {
        ...
        // Create sample users with professional Yemeni context
        $userEmails = ['admin@kindergarten.ye', 'principal@kindergarten.ye', 'accountant@kindergarten.ye', 'staff@kindergarten.ye'];

        // Skip creating users if any already exist
        $existingUsersCount = User::whereIn('email', $userEmails)->count();
        ...
        if ($existingGuardiansCount === 0) {
            ...
            foreach ($children as $child) {
                ...
                $mother = Parents::create([
                    ...
                    'phone' => '+96777'.rand(1000000, 9999999),
                    'relation' => 'Mother',
                    'occupation' => ['معلمة', 'طبيبة', 'محاسبة', 'مديرة', 'موظفة حovernmentية', 'ربة منزل', 'ممرضة'][rand(0, 6)],
                    'address' => "$city - $district",
            ...
            foreach ($children as $child) {
                ...
                for ($i = 0; $i < rand(3, 5); $i++) {
                    Grade::create([
                        ...
                        'evaluator_id' => $teachers[array_rand($teachers)]->id,
                        'created_at' => Carbon::now(),
                        'updated_at' => Carbon::now(),
                    ]);
                }
