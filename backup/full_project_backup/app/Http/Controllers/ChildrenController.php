...
class ChildrenController extends Controller
{
    ...
    protected function exportToExcel($data)
    {
        ...
        // Set the title
        $sheet->setTitle(__('kindergarten.children.title'));
        $sheet->setCellValue('A1', __('kindergarten.children.title'));
        ...
        if ($data->count() > 0) {
            ...
            foreach ($firstItem->getAttributes() as $key => $value) {
                if (! in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at'])) {
                    $headers[] = __('kindergarten.children.'.$key);
                }
            }
        }
    ...
    public function create()
    {
        ...
        return view('pages.children.create', get_defined_vars());
    }
    ...
    public function store(StoreChildrenRequest $request)
    {
        ...
        return redirect()->route('children.index')->with('success', __('kindergarten.messages.created'));
    }

    public function show($id)
    {
        $this->authorize('view_children');
        $children = \App\Models\Children::with(['class', 'parent', 'second_parent', 'attendances', 'payments'])->findOrFail($id);
    ...
    public function edit($id)
    {
        ...
        return view('pages.children.edit', get_defined_vars());
    }
    ...
    public function update(UpdateChildrenRequest $request, $id)
    {
        ...
        return redirect()->route('children.index')->with('success', __('kindergarten.messages.updated'));
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('children.index')->with('success', __('kindergarten.messages.deleted'));
    }
}
