...
class DashboardContentApiController extends BaseApiController
{
    ...
    {
        $dashboardContents = $this->service->create($request->validated());

        return $this->sendResponse(new DashboardContentResource($dashboardContents), __('dashboard_contents.messages.created'), 201);
    }
    ...
    {
        $dashboardContents = $this->service->find($id);
        if (! $dashboardContents) {
            return $this->sendError(__('global.not_found'));
        }

        return $this->sendResponse(new DashboardContentResource($dashboardContents), __('dashboard_contents.messages.retrieved'));
    }
    ...
    {
        $dashboardContents = $this->service->update($id, $request->validated());

        return $this->sendResponse(new DashboardContentResource($dashboardContents), __('dashboard_contents.messages.updated'));
    }
