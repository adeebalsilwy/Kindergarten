...
class FeeController extends Controller
{
    ...
    protected function exportToExcel($data)
    {
        ...
        // Set the title
        $sheet->setTitle(__('kindergarten.fees.title'));
        $sheet->setCellValue('A1', __('kindergarten.fees.title'));
        ...
        if ($data->count() > 0) {
            ...
            foreach ($firstItem->getAttributes() as $key => $value) {
                if (! in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at'])) {
                    $headers[] = __('kindergarten.fees.fields.'.$key);
                }
            }
        }
    ...
    public function store(StoreFeeRequest $request)
    {
        ...
        return redirect()->route('fees.index')->with('success', __('kindergarten.messages.created'));
    }

    public function show($id)
    {
        $this->authorize('view_fees');
        $fees = $this->service->find($id);
    ...
    public function update(UpdateFeeRequest $request, $id)
    {
        ...
        return redirect()->route('fees.index')->with('success', __('kindergarten.messages.updated'));
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('fees.index')->with('success', __('kindergarten.messages.deleted'));
    }
}
