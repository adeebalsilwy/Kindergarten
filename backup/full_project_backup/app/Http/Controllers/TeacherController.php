...
class TeacherController extends Controller
{
    ...
    protected function exportToExcel($data)
    {
        ...
        // Set the title
        $sheet->setTitle(__('kindergarten.teachers.title'));
        $sheet->setCellValue('A1', __('kindergarten.teachers.title'));
        ...
        if ($data->count() > 0) {
            ...
            foreach ($firstItem->getAttributes() as $key => $value) {
                if (! in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at'])) {
                    $headers[] = __('kindergarten.teachers.'.$key);
                }
            }
        }
    ...
    public function store(StoreTeacherRequest $request)
    {
        ...
        return redirect()->route('teachers.index')->with('success', __('kindergarten.messages.created'));
    }

    public function show($id)
    {
        $this->authorize('view_teachers');
        $teachers = \App\Models\Teacher::with(['classes.students'])->findOrFail($id);
    ...
    public function update(UpdateTeacherRequest $request, $id)
    {
        ...
        return redirect()->route('teachers.index')->with('success', __('kindergarten.messages.updated'));
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('teachers.index')->with('success', __('kindergarten.messages.deleted'));
    }
}
