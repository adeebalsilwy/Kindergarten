...
class FinancialReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $financialReports = $query->paginate(15);

        return view('pages.financial_reports.index', compact('financialReports'));
    }
    ...
    {
        $this->authorize('export_financial_reports');
        $data = $query->get();
    ...
    protected function exportToExcel($data)
    {
        ...
        // Set the title
        $sheet->setTitle(__('financial_reports.title'));
        $sheet->setCellValue('A1', __('financial_reports.title'));
        ...
        if ($data->count() > 0) {
            ...
            foreach ($firstItem->getAttributes() as $key => $value) {
                if (! in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at'])) {
                    $headers[] = __('financial_reports.fields.'.$key);
                }
            }
        }
        ...
        $fileName = 'FinancialReport_export_'.date('Y-m-d_H-i-s').'.xlsx';
    ...
    public function store(StoreFinancialReportRequest $request)
    {
        ...
        return redirect()->route('financial_reports.index')->with('success', __('financial_reports.messages.created'));
    }
    ...
    public function show($id)
    {
        ...
        return view('pages.financial_reports.show', compact('financialReports'));
    }
    ...
    public function update(UpdateFinancialReportRequest $request, $id)
    {
        ...
        return redirect()->route('financial_reports.index')->with('success', __('financial_reports.messages.updated'));
    }

    public function destroy($id)
    {
        $this->authorize('delete_financial_reports');
        $this->service->delete($id);

        return redirect()->route('financial_reports.index')->with('success', __('financial_reports.messages.deleted'));
    }
}
