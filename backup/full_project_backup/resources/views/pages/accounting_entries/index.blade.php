                ...
                <x-base.table.tbody>
                    @forelse($accountingEntries as $accountingEntries)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $accountingEntries->debit }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $accountingEntries->credit }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $accountingEntries->entry_date }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $accountingEntries->reference }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $accountingEntries->account_type }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_accounting_entries')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('accounting_entries.show', $accountingEntries->id) }}" size="sm" class="mr-2">
                                    ...
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('accounting_entries.edit', $accountingEntries->id) }}" size="sm" class="mr-2">
                                    ...
                                    <form action="{{ route('accounting_entries.destroy', $accountingEntries->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
