...
@endphp
                ...
                <x-base.table.tbody>
                    @forelse($commandLogs as $commandLogs)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $commandLogs->command }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $commandLogs->parameters }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $commandLogs->status }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_command_logs')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('command_logs.show', $commandLogs->id) }}" size="sm" class="mr-2">
                                    ...
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('command_logs.edit', $commandLogs->id) }}" size="sm" class="mr-2">
                                    ...
                                    <form action="{{ route('command_logs.destroy', $commandLogs->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="intro-y col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">
                        @php
                            $recentCount = $commandLogs->where('created_at', '>=', now()->subDays(7))->count();
                        @endphp
                        {{ $recentCount }}
                    </div>
            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">
                        @php
                            $todayCount = $commandLogs->whereDate('created_at', today())->count();
                        @endphp
                        {{ $todayCount }}
                    </div>
