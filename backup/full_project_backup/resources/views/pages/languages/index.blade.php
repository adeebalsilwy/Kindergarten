...
@endphp
                ...
                <x-base.table.tbody>
                    @forelse($languages as $languages)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $languages->name }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $languages->code }}</x-base.table.td>
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $languages->is_rtl ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $languages->is_rtl ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $languages->is_rtl ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $languages->is_active ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $languages->is_active ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $languages->is_active ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            ...
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_languages')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('languages.show', $languages->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('edit_languages')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('languages.edit', $languages->id) }}" size="sm" class="mr-2">
                                    ...
                                    <form action="{{ route('languages.destroy', $languages->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="intro-y col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">
                        @php
                            $recentCount = $languages->where('created_at', '>=', now()->subDays(7))->count();
                        @endphp
                        {{ $recentCount }}
                    </div>
            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">
                        @php
                            $todayCount = $languages->whereDate('created_at', today())->count();
                        @endphp
                        {{ $todayCount }}
                    </div>
