...
@section('head')
    <title>{{ __('kindergarten.grades.title') }} - {{ config('app.name') }}</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('kindergarten.grades.title') }}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            @can('create_grades')
            <x-base.button variant="primary" as="a" href="{{ route('grades.create') }}" class="ml-2 flex items-center">
                <x-base.lucide icon="Plus" class="w-4 h-4 mr-2" />
                {{ __('kindergarten.grades.add_new') }}
            </x-base.button>
            @endcan
        </div>
    </div>
...
@endphp
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('kindergarten.grades.student') }}</x-base.table.th>
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('kindergarten.grades.subject') }}</x-base.table.th>
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('kindergarten.grades.score') }}</x-base.table.th>
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('kindergarten.grades.date') }}</x-base.table.th>
                ...
                <x-base.table.tbody>
                    @forelse($grades as $grades)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $grades->child_id }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $grades->subject }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $grades->score }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $grades->date->format('Y-m-d') }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_grades')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('grades.show', $grades->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('edit_grades')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('grades.edit', $grades->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('delete_grades')
                                    <form action="{{ route('grades.destroy', $grades->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="intro-y col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">
                        @php
                            $recentCount = $grades->where('created_at', '>=', now()->subDays(7))->count();
                        @endphp
                        {{ $recentCount }}
                    </div>
            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">
                        @php
                            $todayCount = $grades->whereDate('created_at', today())->count();
                        @endphp
                        {{ $todayCount }}
                    </div>
