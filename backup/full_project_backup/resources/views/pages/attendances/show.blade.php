...
@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('Attendance.show') }}</h2>
    </div>
        ...
        <div class="intro-y col-span-12 lg:col-span-10">
            <div class="intro-y box p-5">
                <div class="grid grid-cols-12 gap-4">
                    ...
                    @foreach($attendances->toArray() as $key => $value)
                        @if(!in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at']))
                            <div class="col-span-12 sm:col-span-6">
                                <x-base.form-label>{{ __('Attendance.fields.'.$key) }}</x-base.form-label>
                                <div class="mt-2">
                                    @if(is_bool($value))
                ...
                <div class="flex justify-end mt-5">
                    <a href="{{ route('attendances.index') }}" class="btn btn-outline-secondary w-24 mr-1">{{ __('global.back') }}</a>
                    <a href="{{ route('attendances.edit', $attendances->id) }}" class="btn btn-primary w-24">{{ __('global.edit') }}</a>
                </div>
            </div>
        </div>
    </div>
