...
@endphp
                ...
                <x-base.table.tbody>
                    @forelse($dashboardContents as $dashboardContents)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $dashboardContents->section }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $dashboardContents->key }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $dashboardContents->content }}</x-base.table.td>
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $dashboardContents->is_active ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $dashboardContents->is_active ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $dashboardContents->is_active ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            <x-base.table.td class="text-center">{{ $dashboardContents->order }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $dashboardContents->metadata }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_dashboard_contents')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('dashboard_contents.show', $dashboardContents->id) }}" size="sm" class="mr-2">
                                    ...
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('dashboard_contents.edit', $dashboardContents->id) }}" size="sm" class="mr-2">
                                    ...
                                    <form action="{{ route('dashboard_contents.destroy', $dashboardContents->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
