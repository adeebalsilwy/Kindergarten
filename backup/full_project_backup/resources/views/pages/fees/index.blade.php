...
@section('head')
    <title>{{ __('kindergarten.fees.list') }} - {{ config('app.name') }}</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('kindergarten.fees.list') }}</h2>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <x-base.table class="table-report -mt-2">
                <x-base.table.thead>
                    <x-base.table.tr>
                        <x-base.table.th class="whitespace-nowrap">{{ __('kindergarten.fees.name') }}</x-base.table.th>
                        <x-base.table.th class="whitespace-nowrap text-center">{{ __('kindergarten.fees.amount') }}</x-base.table.th>
                        <x-base.table.th class="whitespace-nowrap text-center">{{ __('kindergarten.fees.status') }}</x-base.table.th>
                ...
                <x-base.table.tbody>
                    @forelse($fees as $fee)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td>
                                <div class="font-medium whitespace-nowrap">{{ $fee->name }}</div>
                            </x-base.table.td>
                            <x-base.table.td class="text-center">{{ number_format($fee->amount, 2) }}</x-base.table.td>
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $fee->is_active ? 'text-success' : 'text-danger' }}">
                                    <x-base.lucide icon="{{ $fee->is_active ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" />
                                    {{ $fee->is_active ? __('kindergarten.fees.active') : __('kindergarten.fees.inactive') }}
                                </div>
                            </x-base.table.td>
                        ...
                        <x-base.table.tr>
                            <x-base.table.td colspan="4" class="text-center py-10">
                                <div class="flex flex-col items-center justify-center">
                                    <x-base.lucide icon="CreditCard" class="w-16 h-16 text-gray-400 mb-4" />
