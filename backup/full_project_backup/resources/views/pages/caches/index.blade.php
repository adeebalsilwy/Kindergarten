    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                        ...
                        <div class="flex gap-2">
                            <x-base.button variant="outline-primary" as="a" href="{{ route('caches.export.pdf') }}" class="flex items-center">
                                <x-base.lucide icon="FileText" class="w-4 h-4 mr-2" />
                                {{ __('global.export_pdf') }}
                            </x-base.button>
                            <x-base.button variant="outline-success" as="a" href="{{ route('caches.export.excel') }}" class="flex items-center">
                                <x-base.lucide icon="FileSpreadsheet" class="w-4 h-4 mr-2" />
                                {{ __('global.export_excel') }}
                            </x-base.button>
                        </div>
            <x-base.button variant="primary" as="a" href="{{ route('caches.create') }}" class="ml-2 flex items-center">
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <x-base.table class="table-report -mt-2">
                <x-base.table.thead>
                    <x-base.table.tr>
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('caches.fields.name') }}</x-base.table.th>
                ...
                <x-base.table.tbody>
                    @forelse($caches as $cache)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $cache->name }}</x-base.table.td>

                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('caches.show', $cache->id) }}" size="sm" class="mr-2">
                                        <x-base.lucide icon="Eye" class="w-4 h-4 mr-1" />
                                        {{ __('global.view') }}
                                    </x-base.button>
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('caches.edit', $cache->id) }}" size="sm" class="mr-2">
                                        <x-base.lucide icon="Pencil" class="w-4 h-4 mr-1" />
                                        {{ __('global.edit') }}
                                    </x-base.button>
                                    <form action="{{ route('caches.destroy', $cache->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <x-base.table.tr>
                            <x-base.table.td colspan="{{ count(array_filter(explode("\n", trim('                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('caches.fields.name') }}</x-base.table.th>
')))) + 1 }}" class="text-center py-10">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <x-base.button variant="primary" as="a" href="{{ route('caches.create') }}" class="mt-4">
                                        <x-base.lucide icon="Plus" class="w-4 h-4 mr-2" />
                                        {{ __('Cache.add_new') }}
                                    </x-base.button>
                                </div>
