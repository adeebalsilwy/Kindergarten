...
class TestModelController extends Controller
{
    ...
    {
        $this->authorize('view_test_models');
        ...
        $testModels = $query->paginate(15);

        return view('pages.test_models.index', compact('testModels'));
    }
    ...
    {
        $this->authorize('export_test_models');
        $data = $query->get();
    ...
    {
        $pdf = Gpdf::loadView('pages.test_models.export-pdf', ['data' => $data]);

        return $pdf->download('TestModel_export_'.date('Y-m-d_H-i-s').'.pdf');
    }
    ...
    protected function exportToExcel($data)
    {
        ...
        // Set the title
        $sheet->setTitle(__('test_models.title'));
        $sheet->setCellValue('A1', __('test_models.title'));
        ...
        if ($data->count() > 0) {
            ...
            foreach ($firstItem->getAttributes() as $key => $value) {
                if (! in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at'])) {
                    $headers[] = __('test_models.fields.'.$key);
                }
            }
        }
        ...
        $fileName = 'TestModel_export_'.date('Y-m-d_H-i-s').'.xlsx';
    ...
    public function store(StoreTestModelRequest $request)
    {
        ...
        return redirect()->route('test_models.index')->with('success', __('test_models.messages.created'));
    }
    ...
    public function show($id)
    {
        ...
        return view('pages.test_models.show', compact('testModel'));
    }
    ...
    public function update(UpdateTestModelRequest $request, $id)
    {
        ...
        return redirect()->route('test_models.index')->with('success', __('test_models.messages.updated'));
    }

    public function destroy($id)
    {
        $this->authorize('delete_test_models');
        $this->service->delete($id);

        return redirect()->route('test_models.index')->with('success', __('test_models.messages.deleted'));
    }
}
