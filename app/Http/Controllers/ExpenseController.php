...
class ExpenseController extends Controller
{
    ...
    protected function exportToExcel($data)
    {
        ...
        // Set the title
        $sheet->setTitle(__('expenses.title'));
        $sheet->setCellValue('A1', __('expenses.title'));
        ...
        if ($data->count() > 0) {
            ...
            foreach ($firstItem->getAttributes() as $key => $value) {
                if (! in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at'])) {
                    $headers[] = __('expenses.fields.'.$key);
                }
            }
        }
        ...
        $fileName = 'Expense_export_'.date('Y-m-d_H-i-s').'.xlsx';
    ...
    public function update(UpdateExpenseRequest $request, $id)
    {
        ...
        return redirect()->route('expenses.index')->with('success', __('expenses.messages.updated'));
    }
