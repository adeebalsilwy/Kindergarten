    ...
    public function handle()
    {
        try {
            ...
            if ($this->shouldGenerate('model', $components)) {
                $this->info('Generating Professional Model...');
                ...
                $hidden = $this->generateHiddenFields($fields);
                $dates = $this->generateDateFields($fields);
                $appends = $this->generateAppends($fields);

                $relationshipResult = $this->generateProfessionalRelationships($tableName);
                $relationships = $relationshipResult['code'];
                $relationshipMetadata = $relationshipResult['metadata'];

                // Generate enhanced model with professional conventions
                $modelContent = $this->generateEnhancedModel(
                    $modelName,
                    $fillable,
                    $casts,
                    $hidden,
    ...
    protected function generateTableBody($fields, $variableName)
    {
        ...
        foreach ($fields as $field => $type) {
            ...
            if ($type === 'boolean') {
                ...
                $body .= "                            <x-base.table.td class=\"text-center\">{{ \${$variableName}->{$field} ? \${$variableName}->{$field}->format('Y-m-d') : '-' }}</x-base.table.td>\n";
            } elseif ($field === 'password') {
                $body .= "                            <x-base.table.td class=\"text-center\">********</x-base.table.td>\n";
            } else {
...
Route::get('{$routePrefix}/export/excel', [{$controllerName}::class, 'exportExcel'])->name('{$routePrefix}.export.excel')->middleware('auth');";
    ...
    /**
     * Generate enhanced model with professional Laravel conventions
     */
    protected function generateEnhancedModel($modelName, $fillable, $casts, $hidden, $dates, $appends, $relationships, $tableName)
    {
        $content = "<?php
...
";
        ...
        // Add soft deletes if deleted_at field exists
        if (str_contains($fillable, 'deleted_at')) {
...
';
    ...
    /**
     * Generate hidden fields for the model
     */
    protected function generateHiddenFields($fields)
    {
        $hidden = '';
        $sensitiveFields = ['password', 'remember_token', 'api_token', 'token', 'secret'];
...
";
    ...
    /**
     * Generate date fields for the model
     */
    protected function generateDateFields($fields)
    {
        $dates = '';

        foreach ($fields as $field => $type) {
            if (str_contains($field, '_at') && $type === 'timestamp') {
                $dates .= "        '{$field}',
";
            }
        }
    ...
    /**
     * Generate appends for the model
     */
    protected function generateAppends($fields)
    {
        $appends = '';
...
";
     ...
     */
    protected function generateFileContent($targetPath, $content, $force = false)
    {
        if (File::exists($targetPath) && ! $force) {
            $this->warn("File already exists: {$targetPath}. Use --force to overwrite.");
    ...
    /**
     * Generate professional relationships with proper type detection
     */
    protected function generateProfessionalRelationships($tableName)
    {
        $relationships = '';
        $metadata = [];
        ...
        foreach ($foreignKeys as $foreignKey) {
            $relatedTable = $foreignKey['referenced_table'];
            $relatedModel = Str::studly(Str::singular($relatedTable));
            $foreignKeyColumn = $foreignKey['column'];

            // Determine relationship type based on column name
            if (str_ends_with($foreignKeyColumn, '_id')) {
                $methodName = str_replace('_id', '', $foreignKeyColumn);
                $methodName = Str::camel($methodName);
            } else {
                $methodName = Str::camel(Str::singular($relatedTable));
            }

            // Check if this might be a many-to-many relationship
            if ($this->isManyToManyRelationship($tableName, $relatedTable)) {
                $relationships .= '    public function '.Str::plural($methodName)."()
    {
...
";
                ...
                $metadata[] = [
                    'type' => 'belongsToMany',
                    'method' => Str::plural($methodName),
                    'related_model' => $relatedModel,
                    'variable_name' => Str::plural($methodName),
                ];
            } else {
                $relationships .= "    public function {$methodName}()
    {
        return \$this->belongsTo({$relatedModel}::class, '{$foreignKeyColumn}');
    }
...
";
                ...
                $metadata[] = [
                    ...
                    'method' => $methodName,
                    'related_model' => $relatedModel,
                    'variable_name' => Str::plural(Str::camel($relatedModel)),
                ];
            }
        }

        // Check for hasMany relationships (reverse of belongsTo)
        ...
        foreach ($reverseRelations as $relation) {
            ...
            $methodName = Str::plural(Str::camel(Str::singular($relatedTable)));
...
";
        ...
        try {
            ...
            foreach ($foreignKeys as $foreignKey) {
                $result[] = [
                    ...
                    'referenced_column' => $foreignKey->getForeignColumns()[0],
                ];
            }
     ...
     */
    protected function isManyToManyRelationship($table1, $table2)
    {
        // Look for pivot tables (usually named as table1_table2 or table2_table1)
        $pivotTable1 = "{$table1}_{$table2}";
        $pivotTable2 = "{$table2}_{$table1}";

        try {
            return Schema::hasTable($pivotTable1) || Schema::hasTable($pivotTable2);
        } catch (\Exception $e) {
            return false;
        }
    }
     ...
     */
    protected function getReverseRelationships($tableName)
    {
        $result = [];
