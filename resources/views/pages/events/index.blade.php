...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('Event.list') }}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            @can('create_events')
            <x-base.button variant="primary" as="a" href="{{ route('events.create') }}" class="ml-2 flex items-center">
                    ...
                    @forelse($events as $event)
                        <x-base.table.tr class="intro-x">
                            ...
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $event->requires_confirmation ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $event->requires_confirmation ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $event->requires_confirmation ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $event->is_public ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $event->is_public ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $event->is_public ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $event->is_recurring ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $event->is_recurring ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $event->is_recurring ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            ...
                            <x-base.table.td class="text-center">
                                <div class="flex items-center justify-center {{ $event->send_reminders ? 'text-success' : 'text-danger' }}"> <x-base.lucide icon="{{ $event->send_reminders ? 'CheckSquare' : 'XSquare' }}" class="w-4 h-4 mr-2" /> {{ $event->send_reminders ? __('global.yes') : __('global.no') }} </div>
                            </x-base.table.td>
                            ...
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_events')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('events.show', $event->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('edit_events')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('events.edit', $event->id) }}" size="sm" class="mr-2">
