...
@section('head')
    <title>{{ __('Teacher.edit') }} - {{ config('app.name') }}</title>
    <style>
...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium me-auto">{{ __('Teacher.edit') }}</h2>
    <div class="flex gap-2">
        <x-base.button variant="outline-secondary" as="a" href="{{ route('teachers.index') }}">
...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12">
        ...
        <form action="{{ route('teachers.update', $teacher->id) }}" method="POST" enctype="multipart/form-data" class="intro-y box p-5">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-between mt-5">
                <div class="flex gap-2">
                    <x-base.button type="button" variant="outline-secondary" as="a" href="{{ route('teachers.index') }}" class="me-1">
                        <x-base.lucide icon="ArrowLeft" class="w-4 h-4 me-2" />
                        {{ __('global.back') }}
                    </x-base.button>
    ...
    function fillDemoData() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Personal Info
                document.querySelector('input[name="name"]').value = 'Updated Teacher Name';
                document.querySelector('input[name="email"]').value = 'updated.teacher@example.com';
                document.querySelector('input[name="phone"]').value = '01234567899';
                document.querySelector('input[name="date_of_birth"]').value = '1988-08-20';
                document.querySelector('select[name="gender"]').value = 'female';
                document.querySelector('textarea[name="address"]').value = '456 Updated Street, New Area, City';
                
                // Professional Info
                document.querySelector('input[name="qualification"]').value = 'PhD in Education';
                document.querySelector('input[name="specialization"]').value = 'Science';
                document.querySelector('input[name="experience_years"]').value = '12';
                document.querySelector('textarea[name="bio"]').value = 'Highly qualified science teacher with extensive experience in curriculum development.';
                
                // Employment Info
                document.querySelector('input[name="hire_date"]').value = new Date().toISOString().split('T')[0];
                document.querySelector('input[name="salary"]').value = '6000';
