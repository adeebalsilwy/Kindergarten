...
@section('head')
    <title>{{ __('Role.show') }} - Laravel</title>
@endsection

@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('Role.show') }}</h2>
    </div>
        ...
        <div class="intro-y col-span-12 lg:col-span-10">
            <div class="intro-y box p-5">
                    ...
                    @foreach($roles->toArray() as $key => $value)
                        @if(!in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at']))
                            <div class="col-span-12 sm:col-span-6">
                                <x-base.form-label>{{ __('Role.fields.'.$key) }}</x-base.form-label>
                                <div class="mt-2">
                                    @if(is_bool($value))
                ...
                <div class="flex justify-end mt-5">
                    <a href="{{ route('roles.index') }}" class="btn btn-outline-secondary w-24 mr-1">{{ __('global.back') }}</a>
                    <a href="{{ route('roles.edit', $roles->id) }}" class="btn btn-primary w-24">{{ __('global.edit') }}</a>
                </div>
            </div>
        </div>
    </div>
