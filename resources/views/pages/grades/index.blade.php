...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('Grade.list') }}</h2>
...
@endphp
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('grades.fields.child_id') }}</x-base.table.th>
                ...
                <x-base.table.tbody>
                    @forelse($grades as $grade)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $grade->child_id ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $grade->subject ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $grade->score ?? '-' }}</x-base.table.td>
                            ...
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_grades')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('grades.show', $grade->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('edit_grades')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('grades.edit', $grade->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('delete_grades')
                                    <form action="{{ route('grades.destroy', $grade->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
