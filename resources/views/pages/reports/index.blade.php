...
@section('head')
    <title>{{ __('Report.list') }} - Laravel</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">{{ __('Report.list') }}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            @can('export_reports')
            
                        <div class="flex gap-2">
                            <x-base.button variant="outline-primary" as="a" href="{{ route('reports.export.pdf') }}" class="flex items-center">
                                <x-base.lucide icon="FileText" class="w-4 h-4 mr-2" />
                                {{ __('global.export_pdf') }}
                            </x-base.button>
                            <x-base.button variant="outline-success" as="a" href="{{ route('reports.export.excel') }}" class="flex items-center">
            ...
            @can('create_reports')
            <x-base.button variant="primary" as="a" href="{{ route('reports.create') }}" class="ml-2 flex items-center">
...
@endphp
                            <x-base.table.th class="whitespace-nowrap text-center">{{ __('reports.fields.name') }}</x-base.table.th>
                        ...
                        <x-base.table.tr class="intro-x">
                            ...
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_reports')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('reports.show', $report->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('edit_reports')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('reports.edit', $report->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('delete_reports')
                                    <form action="{{ route('reports.destroy', $report->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <x-base.table.tr>
                            <x-base.table.td colspan="{{ 1 + ($canEdit || $canDelete || $canView ? 1 : 0) }}" class="text-center py-10">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <x-base.button variant="primary" as="a" href="{{ route('reports.create') }}" class="mt-4">
                                        <x-base.lucide icon="Plus" class="w-4 h-4 mr-2" />
                                        {{ __('Report.add_new') }}
                                    </x-base.button>
                                </div>
        ...
        <div class="intro-y col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">{{ $reports->count() }}</div>
