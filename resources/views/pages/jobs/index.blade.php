...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium me-auto">{{ __('Job.list') }}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            @can('create_jobs')
            <x-base.button variant="primary" as="a" href="{{ route('jobs.create') }}" class="ms-2 flex items-center">
                ...
                <x-base.table.tbody>
                    @forelse($jobs as $job)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $job->queue ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->attempts ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->reserved_at ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->available_at ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->name ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->total_jobs ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->pending_jobs ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->failed_jobs ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->options ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->cancelled_at ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->finished_at ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->uuid ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $job->failed_at ? $job->failed_at->format('Y-m-d') : '-' }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_jobs')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('jobs.show', $job->id) }}" size="sm" class="me-2">
                                    ...
                                    @can('edit_jobs')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('jobs.edit', $job->id) }}" size="sm" class="me-2">
                                    ...
                                    @can('delete_jobs')
                                    <form action="{{ route('jobs.destroy', $job->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <x-base.table.tr>
                        <x-base.table.td colspan="{{ 13 + ($canEdit || $canDelete || $canView ? 1 : 0) }}" class="text-center py-10">
