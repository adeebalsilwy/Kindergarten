                ...
                <form action="{{ route('attendances.store') }}" method="POST" id="attendanceForm">
                    ...
                    <div class="grid grid-cols-12 gap-4 gap-y-5">
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <x-base.form-input id="date" type="date" name="date" value="{{ old('date', $attendance->date ? \Carbon\Carbon::parse($attendance->date)->format('Y-m-d') : now()->format('Y-m-d')) }}" class="mt-2" />
                            @error('date')
                                <div class="text-danger mt-2 text-sm">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-12">
                            ...
                            <x-base.form-textarea id="notes" name="notes" rows="3" class="mt-2 resize-none" placeholder="{{ __('attendances.fields.notes_placeholder') ?? '' }}">{{ old('notes', $attendance->notes ?? '') }}</x-base.form-textarea>
                            @error('notes')
                                <div class="text-danger mt-2 text-sm">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-span-12" id="absence_reason_container">
                            ...
                            <x-base.form-textarea id="absence_reason" name="absence_reason" rows="2" class="mt-2 resize-none" placeholder="{{ __('attendances.fields.absence_reason_placeholder') ?? '' }}">{{ old('absence_reason', $attendance->absence_reason ?? '') }}</x-base.form-textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            generateBtn.addEventListener('click', function() {
                ...
                const options = Object.keys(childSelect.options);
                if (options.length > 1) {
                    const randomChild = options[Math.floor(Math.random() * (options.length - 1)) + 1];
                ...
                document.getElementById('status').tomselect.setValue(randomStatus);

                if (randomStatus === 'present' || randomStatus === 'late') {
                    document.getElementById('check_in').value = randomStatus === 'late' ? '08:45' : '08:00';
                    document.getElementById('check_out').value = '14:00';
                    document.getElementById('notes').value = 'Testing dummy data';
                    document.getElementById('absence_reason').value = '';
                } else {
                    document.getElementById('check_in').value = '';
                    document.getElementById('check_out').value = '';
                    document.getElementById('notes').value = 'Testing dummy data';
                    document.getElementById('absence_reason').value = 'Fever or personal reasons';
                }
