...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            @can('create_expenses')
            <x-base.button variant="primary" as="a" href="{{ route('expenses.create') }}" class="ms-2 flex items-center">
                ...
                <x-base.table.tbody>
                    @forelse($expenses as $expense)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $expense->title ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->amount ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->expense_date ? $expense->expense_date->format('Y-m-d') : '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->vendor ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->receipt_number ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->payment_method ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->reference_number ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->status ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->created_by ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $expense->assigned_to ?? '-' }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_expenses')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('expenses.show', $expense->id) }}" size="sm" class="me-2">
                                    ...
                                    @can('edit_expenses')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('expenses.edit', $expense->id) }}" size="sm" class="me-2">
                                    ...
                                    @can('delete_expenses')
                                    <form action="{{ route('expenses.destroy', $expense->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <x-base.table.tr>
                            <x-base.table.td colspan="{{ 10 + ($canEdit || $canDelete || $canView? 1 : 0) }}" class="text-center py-10">
