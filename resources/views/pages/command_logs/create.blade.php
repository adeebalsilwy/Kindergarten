...
@section('head')
    <title>{{ __('CommandLog.add_new') }} - Laravel</title>
@endsection

@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium me-auto">{{ __('CommandLog.add_new') }}</h2>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-10">
            <div class="intro-y box p-5">
                <form action="{{ route('command_logs.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-12 gap-4">
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <x-base.tom-select name="status" class="mt-2">
                                ...
                                <option value="inactive" {{ old('status', $commandLog->status ?? '') == 'inactive' ? 'selected' : '' }}>{{ __('global.inactive') }}</option>
                                <option value="pending" {{ old('status', $commandLog->status ?? '') == 'pending' ? 'selected' : '' }}>{{ __('global.pending') }}</option>
                                <option value="draft" {{ old('status', $commandLog->status ?? '') == 'draft' ? 'selected' : '' }}>{{ __('global.draft') }}</option>
                            </x-base.tom-select>
                        </div>
                        <div class="col-span-12">
                    ...
                    <div class="flex justify-end mt-5">
                        <a href="{{ route('command_logs.index') }}" class="btn btn-outline-secondary w-24 me-1">{{ __('global.cancel') }}</a>
