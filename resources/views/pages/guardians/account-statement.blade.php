...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <x-base.button variant="outline-primary" as="a" href="{{ route('guardians.export.pdf') }}?report_type=account_statement" class="flex items-center me-2">
                <x-base.lucide icon="FileText" class="w-4 h-4 me-2" />
                {{ __('global.export_pdf') }}
            </x-base.button>
            <x-base.button variant="outline-success" as="a" href="{{ route('guardians.export.excel') }}?report_type=account_statement" class="flex items-center">
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="intro-y col-span-12">
            <div class="box p-5">
                <form method="GET" action="{{ route('guardians.account-statement', $guardian->id) }}">
                ...
                <x-base.table.tbody>
                    @forelse($accountStatement['entries'] as $entry)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ is_string($entry['date']) ? $entry['date'] : $entry['date']->format('Y-m-d') }}</x-base.table.td>
