...
@section('head')
    <title>{{ __('TestModel.list') }} - Laravel</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium me-auto">{{ __('TestModel.list') }}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            @can('export_test_models')
            ...
            @can('create_test_models')
            <x-base.button variant="primary" as="a" href="{{ route('test_models.create') }}" class="ms-2 flex items-center">
...
@endphp
                ...
                <x-base.table.tbody>
                    @forelse($testModels as $testModel)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $testModel->name ?? '-' }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_test_models')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('test_models.show', $testModel->id) }}" size="sm" class="me-2">
                                        <x-base.lucide icon="Eye" class="w-4 h-4 me-1" />
                                        {{ __('global.view') }}
                                    </x-base.button>
                                    @endcan
                                    
                                    @can('edit_test_models')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('test_models.edit', $testModel->id) }}" size="sm" class="me-2">
                                        <x-base.lucide icon="Pencil" class="w-4 h-4 me-1" />
                                        {{ __('global.edit') }}
                                    </x-base.button>
                                    @endcan
                                    
                                    @can('delete_test_models')
        ...
        <div class="intro-y col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-3xl font-bold leading-8 mt-6">{{ $testModels->count() }}</div>
