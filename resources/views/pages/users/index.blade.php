                ...
                <x-base.table.tbody>
                    @forelse($users as $user)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="text-center">{{ $user->name ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $user->email ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $user->email_verified_at ? $user->email_verified_at->format('Y-m-d') : '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">********</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $user->token ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $user->user_id ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $user->ip_address ?? '-' }}</x-base.table.td>
                            <x-base.table.td class="text-center">{{ $user->last_activity ?? '-' }}</x-base.table.td>

                            @if($canEdit || $canDelete || $canView)
                            <x-base.table.td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @can('view_users')
                                    <x-base.button variant="outline-secondary" as="a" href="{{ route('users.show', $user->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('edit_users')
                                    <x-base.button variant="outline-primary" as="a" href="{{ route('users.edit', $user->id) }}" size="sm" class="mr-2">
                                    ...
                                    @can('delete_users')
                                    <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('{{ __('global.confirm_delete') }}')" class="inline">
                                        @csrf
                                        @method('DELETE')
