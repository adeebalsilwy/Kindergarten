...
@section('head')
    <title>{{ __('User.edit') }} - {{ config('app.name') }}</title>
    <style>
        .tabs-navigation {
            display: flex;
            gap: 0.5rem;
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8 mb-8">
        <div class="me-auto">
            <h2 class="text-3xl font-black text-slate-800 dark:text-slate-200 tracking-tight flex items-center">
                ...
                <span class="text-primary">{{ __('User.edit') }}: {{ $user->name }}</span>
            </h2>
        </div>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0 gap-3">
            ...
            <x-base.button variant="outline-secondary" as="a" href="{{ route('users.show', $user->id) }}" class="flex items-center px-6 py-3 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 bg-white dark:bg-darkmode-600 border-0 font-bold">
                        ...
                        <div class="grid grid-cols-12 gap-x-10 gap-y-10">
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <div class="relative group">
                                    ...
                                    <x-base.form-input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" placeholder="{{ __('users.placeholders.name') }}" class="ps-16 py-5 rounded-[1.5rem] border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" />
                                </div>
                            ...
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <div class="relative group">
                                    ...
                                    <x-base.form-input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" placeholder="{{ __('users.placeholders.email') }}" class="ps-16 py-5 rounded-[1.5rem] border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" />
                                </div>
                            ...
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <div class="relative group">
                                    ...
                                    <x-base.form-input type="tel" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" placeholder="{{ __('users.placeholders.phone') }}" class="ps-16 py-5 rounded-[1.5rem] border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" />
                                </div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div class="ms-auto">
                                            <div class="form-check form-switch scale-150">
                                                <x-base.form-input type="checkbox" id="is_active" name="is_active" value="1" class="form-check-input w-14 h-7" {{ old('is_active', $user->is_active) ? 'checked' : '' }} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div id="security" class="tab-content">
                        <div class="flex items-center mb-12 pb-8 border-b border-slate-100 dark:border-darkmode-400">
                            ...
                            <div>
                                ...
                                <p class="text-slate-400 font-bold text-sm mt-1 uppercase tracking-widest">{{ __('global.security_description_edit') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-12 gap-x-10 gap-y-10">
                            <div class="col-span-12 md:col-span-6">
                                ...
                                <div class="relative group">
                                    ...
                                    <x-base.form-input type="password" name="password" id="password" placeholder="{{ __('users.placeholders.password_edit') }}" autocomplete="new-password" class="ps-16 py-5 rounded-[1.5rem] border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-base font-bold" />
                                ...
                                <div class="role-card group p-8 rounded-[2.5rem] border-2 border-slate-100 dark:border-darkmode-400 bg-slate-50/50 dark:bg-darkmode-700/50 hover:border-primary/30 hover:bg-white dark:hover:bg-darkmode-600 transition-all duration-300 cursor-pointer relative overflow-hidden">
                                    <div class="flex items-start relative z-10">
                                        <div class="form-check" onclick="event.stopPropagation()">
                                            <x-base.form-input type="checkbox" id="role_{{ $role->id }}" name="roles[]" value="{{ $role->id }}" class="form-check-input role-checkbox w-8 h-8 border-slate-300 rounded-xl" {{ in_array($role->id, $userRoles) ? 'checked' : '' }} />
                                        </div>
                                        <div class="ms-6">
                                        ...
                                        <div class="p-6 space-y-4 flex-1">
                                            ...
                                            <div class="flex items-center p-4 rounded-2xl hover:bg-primary/5 dark:hover:bg-darkmode-500 transition-all duration-300 group cursor-pointer border border-transparent hover:border-primary/10" onclick="this.querySelector('input').click()">
                                                <div onclick="event.stopPropagation()">
                                                    <x-base.form-input type="checkbox" id="permission_{{ $permission->id }}" name="permissions[]" value="{{ $permission->id }}" class="form-check-input permission-checkbox w-7 h-7 border-slate-300 rounded-xl" data-group="{{ $group }}" {{ in_array($permission->id, $userPermissions) ? 'checked' : '' }} />
                                                </div>
