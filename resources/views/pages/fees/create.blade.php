...
@section('head')
    <title>{{ __('Fee.add_new') }} - Laravel</title>
@endsection

@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium me-auto">{{ __('Fee.add_new') }}</h2>
    </div>
            ...
            <div class="intro-y box p-5">
                <form action="{{ route('fees.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <div class="relative mt-2">
                                ...
                                <x-base.form-input type="number" step="0.01" min="0" name="amount" value="{{ old('amount', $fee->amount ?? '') }}" class="ps-12" />
                            </div>
                            @error('amount')
                                <div class="text-danger mt-2">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="flex justify-end mt-5">
                        <a href="{{ route('fees.index') }}" class="btn btn-outline-secondary w-24 me-1">{{ __('global.cancel') }}</a>
