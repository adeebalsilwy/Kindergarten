...
@section('head')
    <title>{{ __('TestModel.show') }} - Laravel</title>
@endsection
        ...
        <div class="intro-y col-span-12 lg:col-span-10">
            <div class="intro-y box p-5">
                <div class="grid grid-cols-12 gap-4">
                    ...
                    @foreach($testModel->toArray() as $key => $value)
                        @if(!in_array($key, ['id', 'created_at', 'updated_at', 'deleted_at']))
                            <div class="col-span-12 sm:col-span-6">
                                <x-base.form-label>{{ __('TestModel.fields.'.$key) }}</x-base.form-label>
                                <div class="mt-2">
                                    @if(is_bool($value))
                ...
                <div class="flex justify-end mt-5">
                    <a href="{{ route('test_models.index') }}" class="btn btn-outline-secondary w-24 mr-1">{{ __('global.back') }}</a>
                    <a href="{{ route('test_models.edit', $testModel->id) }}" class="btn btn-primary w-24">{{ __('global.edit') }}</a>
                </div>
            </div>
        </div>
    </div>
