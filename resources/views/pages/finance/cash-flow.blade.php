...
<div class="grid grid-cols-12 gap-6 mt-5">
    ...
    <div class="col-span-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white dark:bg-dark-1 shadow rounded-lg p-6">
            ...
            <div class="text-2xl font-bold text-green-600 mt-2">${{ number_format($cashFlow['total_net_cash_flow'] > 0 ? $cashFlow['total_net_cash_flow'] : 0, 2) }}</div>
        </div>
        <div class="bg-white dark:bg-dark-1 shadow rounded-lg p-6">
            <div class="text-gray-600 dark:text-gray-400">Total Expenses</div>
            <div class="text-2xl font-bold text-red-600 mt-2">${{ number_format($cashFlow['total_net_cash_flow'] < 0 ? abs($cashFlow['total_net_cash_flow']) : 0, 2) }}</div>
        </div>
        <div class="bg-white dark:bg-dark-1 shadow rounded-lg p-6">
            <div class="text-gray-600 dark:text-gray-400">Net Cash Flow</div>
            <div class="text-2xl font-bold {{ $cashFlow['total_net_cash_flow'] >= 0 ? 'text-green-600' : 'text-red-600' }} mt-2">
                ${{ number_format($cashFlow['total_net_cash_flow'] ?? 0, 2) }}
            </div>
        </div>
        <div class="bg-white dark:bg-dark-1 shadow rounded-lg p-6">
            <div class="text-gray-600 dark:text-gray-400">Average Daily Flow</div>
            <div class="text-2xl font-bold text-blue-600 mt-2">
                ${{ number_format($cashFlow['average_daily_cash_flow'] ?? 0, 2) }}
            </div>
        </div>
    </div>
