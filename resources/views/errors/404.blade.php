...
@section('head')
    <title>404 - {{ __('global.page_not_found') ?? 'Page Not Found' }}</title>
    <style>
            ...
            <div class="intro-y box error-card p-12 text-center rounded-[3rem] shadow-2xl bg-white/80 dark:bg-darkmode-600/80 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <div class="floating-icon mb-10 inline-flex items-center justify-center w-32 h-32 rounded-[2.5rem] bg-primary/10 text-primary shadow-inner">
                        <x-base.lucide icon="MapPinOff" class="w-16 h-16" />
                    </div>
                    ...
                    <h2 class="text-4xl font-black text-slate-800 dark:text-slate-200 mb-6 tracking-tight">
                        {{ __('global.oops') ?? 'Oops!' }} {{ __('global.page_not_found') ?? 'Page Not Found' }}
                    </h2>
                    <p class="text-slate-500 dark:text-slate-400 text-lg font-bold mb-12 max-w-md mx-auto leading-relaxed">
                        {{ __('global.404_message') ?? 'The page you are looking for might have been removed, had its name changed, or is temporarily unavailable.' }}
                    </p>
