...
@section('head')
    <title>500 - {{ __('global.server_error') ?? 'Server Error' }}</title>
    <style>
            ...
            <div class="intro-y box error-card p-12 text-center rounded-[3rem] shadow-2xl bg-white/80 dark:bg-darkmode-600/80 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <div class="floating-icon mb-10 inline-flex items-center justify-center w-32 h-32 rounded-[2.5rem] bg-danger/10 text-danger shadow-inner">
                        <x-base.lucide icon="ServerCrash" class="w-16 h-16" />
                    </div>

                    <!-- Error Code -->
                    <h1 class="text-9xl font-black text-slate-800 dark:text-slate-200 tracking-tighter mb-4 opacity-10">500</h1>
                    
                    <!-- Message -->
                    <h2 class="text-4xl font-black text-slate-800 dark:text-slate-200 mb-6 tracking-tight">
                        {{ __('global.something_went_wrong') ?? 'Internal Server Error' }}
                    </h2>
                    <p class="text-slate-500 dark:text-slate-400 text-lg font-bold mb-12 max-w-md mx-auto leading-relaxed">
                        {{ __('global.500_message') ?? 'We track these errors automatically, but if the problem persists feel free to contact us. In the meantime, try refreshing.' }}
                    </p>

                    <!-- Actions -->
                    <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <x-base.button type="button" onclick="window.location.reload()" variant="outline-secondary" class="px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-xs border-2 hover:bg-slate-100 transition-all duration-300 w-full sm:w-auto">
                            <x-base.lucide icon="RefreshCw" class="w-4 h-4 me-2" />
                            {{ __('global.refresh_page') ?? 'Refresh Page' }}
                        </x-base.button>
